 ...
 * @subpackage parser
 */
class Parser_BBCode extends Parser {
	/**
	 * Parser mode
	 *
	 ...
	 */
	var $_parserMode = PARSER_MODE_SEARCH;
	...
	function _handleStatus ($status, $needle) {
		switch ($status) {
			case 0: // NORMAL TEXT
				...
				if ($needle == '[') {
					$node =& new Parser_BBCode_Node_Element ($this->_cpos);
	...
	function _outputNode (&$node) {
		$output = '';
		if ($node->_type == PARSER_BBCODE_NODE_PARAGRAPH || $node->_type == PARSER_BBCODE_NODE_ELEMENT || $node->_type == PARSER_NODE_ROOT) {
			...
			if ($node->_type == PARSER_BBCODE_NODE_PARAGRAPH) {
				return $this->_paragraphHandling['start_tag'].$output.$this->_paragraphHandling['end_tag'];
			}
			if ($node->_type == PARSER_BBCODE_NODE_ELEMENT) {
				return $node->getReplacement ($output);
			}
			return $output;
		} else if ($node->_type == PARSER_NODE_TEXT) {
			$output = $node->content;
			$before = '';
			$after = '';
			$ol = strlen ($output);
			...
			if ($node->_parent->_type == PARSER_BBCODE_NODE_PARAGRAPH)  {
				$parent =& $node->_parent;
				unset ($node);
				$node =& $parent;
				unset ($parent);
			...
			if ($node->_parent->_type == PARSER_NODE_ROOT) {
				return $before.$this->_applyParsers ($this->_rootContentType, $output).$after;
			}
			if ($node->_parent->_type == PARSER_BBCODE_NODE_ELEMENT) {
	...
	function _modifyTree () {
		...
		for ($i = 0; $i < $nodes_count; $i++) {
			if ($nodes[$i]->_type != PARSER_BBCODE_NODE_PARAGRAPH) {
				continue;
			}
			unset ($parent);
	...
	function _handleParagraphs (&$node) {
		...
		while (count ($node->_children)) {
			...
			for ($i = 0; $i < count ($sub_nodes); $i++) {
				if (!$last_node_was_paragraph ||  ($prevtype == $sub_nodes[$i]->_type && ($i != 0 || $prevtype != PARSER_BBCODE_NODE_ELEMENT))) {
					unset ($paragraph);
					$paragraph =& new Parser_BBCode_Node_Paragraph ();
				}
				$prevtype = $sub_nodes[$i]->_type;
				if ($sub_nodes[$i]->_type != PARSER_BBCODE_NODE_ELEMENT || $sub_nodes[$i]->getFlag ('paragraph_type', 'integer', BBCODE_PARAGRAPH_ALLOW_BREAKUP) != BBCODE_PARAGRAPH_BLOCK_ELEMENT) {
	...
	function _hasParagraphAncestor (&$node) {
		...
		$parent =& $node->_parent;
		if ($parent->_type == PARSER_BBCODE_NODE_PARAGRAPH) {
			return true;
		}
	...
	function &_breakupNodeByParagraphs (&$node) {
		...
		if ($node->_type == PARSER_NODE_TEXT) {
			$cpos = 0;
			while (($npos = strpos ($node->content, $detect_string, $cpos)) !== false) {
				$subnode =& new Parser_Node_Text (substr ($node->content, $cpos, $npos - $cpos), $node->occurredAt + $cpos);
				// copy flags
			...
			$subnode =& new Parser_Node_Text (substr ($node->content, $cpos), $node->occurredAt + $cpos);
			if ($cpos == 0) {
 ...
 * @see Parser_BBCode_Node_Element::_type
 */
define ('PARSER_BBCODE_NODE_ELEMENT', 32);

/**
 * Node type: BBCode Paragraph node
 * @see Parser_BBCode_Node_Paragraph::_type
 */
define ('PARSER_BBCODE_NODE_PARAGRAPH', 33);


/**
 * BBCode Parser paragraph node class
 *
 * @package parser
 */
class Parser_BBCode_Node_Paragraph extends Parser_Node {
	/**
	 ...
	 * @access protected
	 * @var int
	 * @see PARSER_BBCODE_NODE_PARAGRAPH
	 */
	var $_type = PARSER_BBCODE_NODE_PARAGRAPH;
	...
	function matchesCriterium ($criterium, $value) {
		if ($criterium == 'empty') {
			...
			if ($this->_children[0]->_type != PARSER_NODE_TEXT) {
				return false;
			}
...
/**
 * BBCode Parser element node class
 *
 * @package parser
 */
class Parser_BBCode_Node_Element extends Parser_Node {
	/**
	 * The type of this node.
	 * 
	 ...
	 * @access protected
	 * @var int
	 * @see PARSER_BBCODE_NODE_ELEMENT
	 */
	var $_type = PARSER_BBCODE_NODE_ELEMENT;
	 ...
	 * @access protected
	 * @var string
	 * @see Parser_BBCode_Node_Element::name
	 * @see Parser_BBCode_Node_Element::setName
	 * @see Parser_BBCode_Node_Element::appendToName
	 */
	var $_name = '';
	 ...
	 */
	function &duplicate () {
		$newnode =& new Parser_BBCode_Node_Element ($this->occurredAt);
	...
	function &lastChildIfText () {
		...
		if ($ret->_type != PARSER_NODE_TEXT || !$this->_hadCloseTag) {
			// DON'T DO $ret = null WITHOUT unset BEFORE!
			// ELSE WE WILL ERASE THE NODE ITSELF! EVIL!
			if ($ret->_type != PARSER_NODE_TEXT && !$ret->hadCloseTag ()) {
	...
	function &findNextAdjentTextNode () {
		...
		if ($found < $ccount - 1) {
			if ($this->_parent->_children[$found+1]->_type == PARSER_NODE_TEXT) {
				return $this->_parent->_children[$found+1];
			}
			return $ret;
		}
		if ($this->_parent->_type == PARSER_BBCODE_NODE_ELEMENT && !$this->_parent->hadCloseTag ()) {
	...
	function &_findPrevAdjentTextNodeHelper () {
		...
		if ($lastnode === null || $lastnode->_type == PARSER_NODE_TEXT) {
			return $lastnode;
		}
	...
	function validate ($action = 'validate') {
		...
		if ($this->_codeInfo['callback_type'] != 'simple_replace' && $this->_codeInfo['callback_type'] != 'simple_replace_single') {
			...
			if (($this->_codeInfo['callback_type'] == 'usecontent' || $this->_codeInfo['callback_type'] == 'usecontent?' || $this->_codeInfo['callback_type'] == 'callback_replace?') && count ($this->_children) == 1 && $this->_children[0]->_type == PARSER_NODE_TEXT) {
