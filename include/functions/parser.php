 ...
 */
/**
 * Parser mode: Search for the next character
 * @see Parser::_parserMode
 */
define ('PARSER_MODE_SEARCH', 1);
/**
 * Parser mode: Look at each character of the string
 * @see Parser::_parserMode
 */
define ('PARSER_MODE_LOOP', 2);
/**
 * Filter type: Prefilter
 * @see Parser::addFilter, Parser::_prefilters
 */
define ('PARSER_FILTER_PRE', 1);
/**
 * Filter type: Postfilter
 * @see Parser::addFilter, Parser::_postfilters
 */
define ('PARSER_FILTER_POST', 2);

/**
 * Generic parser class
 *
 * This is an abstract class for any type of parser.
 *
 * @subpackage parser
 */
class Parser {
	 ...
	 * @access protected
	 * @var int
	 * @see PARSER_MODE_SEARCH, PARSER_MODE_LOOP
	 */
	 ...
	 * loop mode, {@link Parser::_charactersSearch _charactersSearch}
	 ...
	 * @return bool
	 * @see PARSER_FILTER_PRE, PARSER_FILTER_POST
	 */
	function addFilter ($type, $callback) {
		...
		switch ($type) {
			case PARSER_FILTER_PRE:
				$this->_prefilters[] = $callback;
				break;
			case PARSER_FILTER_POST:
	...
	function parse ($text) {
		...
		if (is_object ($this->_root)) {
			Parser_Node::destroyNode ($this->_root);
		}
		unset ($this->_root);
		$this->_root =& new Parser_Node_Root ();
		...
		$res = Parser_Node::destroyNode ($this->_root);
		if (!$res) {
			$this->_parsing = false;
			return false;
		}
	...
	/**
	 * Searches {@link Parser::_text _text} for every needle that is
 ...
 * @see Parser_Node::_type
 */
define ('PARSER_NODE_UNKNOWN', 0);

/**
 * Node type: Root node
 * @see Parser_Node::_type
 */
define ('PARSER_NODE_ROOT', 1);

/**
 * Node type: Text node
 * @see Parser_Node::_type
 */
define ('PARSER_NODE_TEXT', 2);

/**
 * Global value that is a counter of parser node ids. Compare it to a
 * sequence in databases.
 * @var int
 */
$GLOBALS['__PARSER_NODE_ID'] = 0;

/**
 * Generic parser node class
 *
 * This is an abstract class for any type of node that is used within the
 * parser. General warning: This class contains code regarding references
...
class Parser_Node {
	 ...
	 * @access protected
	 * @var int
	 * @see PARSER_NODE_ROOT, PARSER_NODE_TEXT
	 * @see PARSER_NODE_UNKNOWN
	 */
	var $_type = PARSER_NODE_UNKNOWN;
	 ...
	 * @global __PARSER_NODE_ID
	 */
	function Parser_Node ($occurredAt = -1) {
		$this->_id = $GLOBALS['__PARSER_NODE_ID']++;
	...
	function prependChild (&$node) {
		...
		if ($node->_type == PARSER_NODE_ROOT) {
			return false;
		}
	...
	function appendToLastTextChild ($text) {
		...
		if ($ccount == 0 || $this->_children[$ccount-1]->_type != PARSER_NODE_TEXT) {
			$ntextnode =& new Parser_Node_Text ($text);
	...
	function _dumpToString () {
		if ($this->_type == PARSER_NODE_ROOT) {
			return "root";
		}
...
/**
 * Parser root node class
 *
 * @package parser
 */
class Parser_Node_Root extends Parser_Node {
	/**
	 * The type of this node.
	 * 
	 ...
	 * @access protected
	 * @var int
	 * @see PARSER_NODE_ROOT
	 */
	var $_type = PARSER_NODE_ROOT;
}

/**
 * Parser text node class
 *
 * @package parser
 */
class Parser_Node_Text extends Parser_Node {
	/**
	 * The type of this node.
	 * 
	 ...
	 * @access protected
	 * @var int
	 * @see PARSER_NODE_TEXT
	 */
	var $_type = PARSER_NODE_TEXT;
